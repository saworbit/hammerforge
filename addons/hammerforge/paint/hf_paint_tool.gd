	...
	for dy in range(-r, r + 1):
		for dx in range(-r, r + 1):
			...
			if is_blend:
				...
				layer.set_cell_blend(target, blend_strength)
			else:
...
func _reconcile_heightmap(layer: HFPaintLayer, dirty: Array[Vector2i]) -> void:
	...
	# Heightmap floors from heightmap synth
	var hm_results = heightmap_synth.build_for_chunks(layer, dirty, synth_settings)
	...
	# Walls still generated by flat geometry synth
	var wall_model := geometry.build_for_chunks(layer, dirty, synth_settings)
