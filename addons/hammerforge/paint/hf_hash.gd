...
## The layer field of a generated id, or "" when the id is not one.
##
## Field 3 is the layer and field 4 the chunk, which holds because
## `HFPaintLayerManager.safe_layer_id()` keeps a ":" out of every layer id.
## These are for reading a node generated by an older build; the reconciler
## takes both from the layer it is reconciling rather than parsing them back.
static func layer_tag_from_id(gid: StringName) -> String:
