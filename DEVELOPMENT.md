...
- **Brush-scoped undo steps.** `capture_state()` is the whole level, so at 900 brushes one undo step was 39 ms to take and 2.2 MB to hold (#737). Pass `scope_brush_ids` to `HFUndoHelper.commit()` and the step records only those brushes, through `HFStateSystem.capture_brush_scope()` / `restore_brush_scope()`. The scoped restore looks each brush up by id — no `clear_brushes()`, no reconcile — and applies the record in place when only `transform` and `faces` differ, rebuilding through `create_brush_from_info()` otherwise and putting the brush back at its captured index. The transform commands claim a scope through `plugin_edit_actions.brush_scope()`, which refuses as soon as an entity is in the selection. The displacement commands, the sculpt drag, material paint, bevel, inset, the gizmo resize, the UV spinboxes and a material dropped on a face claim one at their own call sites. Do not read a list of scoped commands from here — `tests/test_scoped_undo_step.gd` is the list, because a command only counts as scoped once it has a case there.
