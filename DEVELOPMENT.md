...
addons/hammerforge/
  ...
  plugin_selection_state.gd EditorSelection sync, owner/group normalization, and action scope guards
  ...
  undo_helper.gd         HFUndoHelper: state-capture undo with collation (merges rapid edits into one step) and scoped steps
  ...
  map_io.gd              .map import/export with parse validation (adapter pattern for multi-format support)
  ...
  systems/               Subsystem classes (RefCounted)
    ...
    hf_file_system.gd      .hflevel/.map/.glTF I/O, FIFO threaded writes, map validation, save failure reporting
...
- **Coordinator boundaries are structural, not line-count targets.** `plugin.gd` should retain EditorPlugin lifecycle, composition, LevelRoot discovery, undo-manager wiring, and short high-level coordination. Cohesive input, overlay, command, HUD, selection, dialog, and tool behavior belongs in focused modules with boundary tests. Do not create artificial forwarding layers solely to reduce the coordinator's line count.
...
- **Sticky LevelRoot discovery.** `plugin.gd` keeps `active_root` sticky: `sync_active_root_from_selection()` does not null it when non-HammerForge nodes are selected. `should_handle_editor_object()` is the narrow bar for retargeting it, claiming only the active `LevelRoot`, HammerForge brushes/entities, and entity subtrees; unrelated scene nodes remain owned by Godot's native editor. `dock.gd` mirrors sticky root discovery without broadening editor-object ownership.
- **A main screen must not handle objects.** `_has_main_screen()` returns true so HammerForge holds a place in the switcher beside 2D, 3D and Script. That makes `_handles()` permanently false. Godot's `EditorData::get_handling_main_editor()` switches the editor to any plugin that declares both, so a `_handles()` that claimed `DraftBrush` threw the user off the 3D view on every brush click. `active_root` is therefore set from `selection_changed` rather than from `_edit()`, which Godot no longer calls.
...
- **Scoped undo steps.** `capture_state()` is the whole level, so at 900 brushes one undo step was 39 ms to take and 2.2 MB to hold (#737). Pass `scope_brush_ids` and `scope_entity_paths` to `HFUndoHelper.commit()` and the step records only those objects, through `HFStateSystem.capture_brush_scope()` / `restore_brush_scope()`. The scoped restore looks each brush up by id — no `clear_brushes()`, no reconcile — and applies the record in place when only `transform` and `faces` differ, rebuilding through `create_brush_from_info()` otherwise and putting the brush back at its captured index. Entities go back through `HFEntitySystem.apply_entity_record()`, which writes the record onto the live node: nothing is freed, so no node path moves and no entity has to be resolved by anything but the path that named it (#761). The four transform commands pass their whole selection, mixed or entity-only. The displacement commands, the sculpt drag, material paint, bevel, inset, the gizmo resize, the UV spinboxes and a material dropped on a face claim one at their own call sites. Do not read a list of scoped commands from here — `tests/test_scoped_undo_step.gd` is the list, because a command only counts as scoped once it has a case there.

- **Taking the "before" state.** Use `HFUndoHelper.capture_scope_or_state(root, ids)` rather than calling `capture_brush_scope()` and falling back by hand. It returns `{"state", "scope_ids"}`, and both go to `commit_completed()` together. They have to travel together: `commit_completed()` picks `restore_brush_scope` from the ids alone, so a caller that asked for a scope, got the whole-level fallback, and still passed its ids would register a level state to be restored as a scope. The commands that create or delete geometry — a decal, a path, a polygon, a dropped brush, a dropped prefab — and floor painting, which writes paint chunks, keep the whole snapshot and should not be scoped. A dropped *material* is not one of those: it textures a face of a brush that is already there, so it scopes like any other face edit.
- **What a scope claims, and what holds it to that.** A scope is a claim that the command changes those objects and no registry, palette or other brush or entity. Nothing in a record can check it, so `tests/test_scoped_undo_step.gd` runs each scoped command between two `capture_state()` calls and asserts `brushes` and `entities` are the only keys that differ. A command that grows a registry write fails there, and the fix is to stop scoping it rather than to widen the scope. Objects that cannot be a scope — an id that does not resolve, a pending or committed cut, a path that is not a managed entity — make `capture_brush_scope()` return `{}` and the caller falls back to the whole snapshot, so a wrong-looking claim costs speed and not correctness. A scope says what it is: `capture_brush_scope()` puts `HFValidation.UNDO_SCOPE_KEY` on the dictionary and `level_state_problem()` refuses anything carrying it, so a scope handed to `restore_state()` is turned away rather than read as a level with nothing in it and freeing every brush it did not name (#768). A step captured before that key existed carries no tag, and `restore_brush_scope()` still reads it. The other direction is not guarded: a whole level handed to `restore_brush_scope()` puts the brushes back and none of the rest, which is why `capture_scope_or_state()` returns the state and the ids together.
...
- **Central signals.** Subscribe to LevelRoot signals (`brush_added`, `brush_removed`, `brush_changed`, `entity_added`, `entity_removed`, `paint_layer_changed`, `material_list_changed`, `face_selection_changed`, `state_saved`, etc.) instead of polling. Subsystems emit these via `root.<signal>.emit(...)`, or `root._emit_or_batch(name, args)` so a multi-object operation flushes them together. `face_selection_changed` emits only when selection actually changes (snapshot comparison in `select_face_at_screen`, plus deleting a brush that owned a selected face). `selection_changed` is declared but never emitted: LevelRoot holds no brush selection, Godot's `EditorSelection` does. A batch must not fabricate one from the ids it just touched.
- **Save failure reporting.** `autosave_failed(error_message)` fires for autosaves, `hflevel_save_failed(path, error_message)` for manual saves, and `hflevel_save_completed(path)` only after the matching worker succeeds. A failed region sidecar reports through the same pair, chosen by whether the save was an autosave. Connect these in the dock to show user-facing warnings.
...
- **Booleans are capped at `HFConvexClip.MAX_BOOLEAN_PLANES` distinct planes.** Every plane is a split of a growing face set, so cost and piece count both climb steeply. A box has six planes and a cylinder sixty-six; a sphere has thousands, because every triangle is its own plane. Shelling one measured sixty seconds and 2,051 brushes before the cap existed. Hollow and carve now check the budget first and refuse with the real number.
- **Convex clipping.** `HFConvexClip` (`hf_convex_clip.gd`) splits a convex solid, described by its faces, along an arbitrary plane, and is the geometry behind both Clip and Carve. It never touches the scene: faces and a `Plane` in, faces out, which is what lets both callers be tested without a LevelRoot. Face splitting is Sutherland-Hodgman in three dimensions with a *three-way* vertex classification — front, back, and on the plane — because a two-way inside test emits an on-plane vertex and then an intersection at the same point, leaving a zero-length edge that stops the piece being closed. That case is not exotic: it is what a diagonal cut through a box's own edges does. The cut surface is rebuilt rather than carried through, which makes it the one face here that can arrive inside out, so it goes through `sort_coplanar_cw()` — the single home for ring ordering, named for the convention because the mathematically natural ordering is the opposite one. Crossing points are deduplicated by distance rather than by grid cell (two crossings either side of a cell boundary both survive a grid dedup and fan into a sliver whose normal is noise), and ring vertices that stand on the line between their neighbours are dropped for the same reason. `is_axis_aligned_box()` lets a caller emit a cut piece as a BOX when it still is one, so the commonest cut of all does not cost the pieces their resize handles.
- **Transform system.** `HFTransformSystem` (`systems/hf_transform_system.gd`) owns rotate, flip and reset rotation for both brushes and entities. Rotation is determinant `+1`, so local face data is untouched and the basis carries the change; the product is re-orthonormalised each call (with any node scale taken off and put back) because a brush accumulates one per key press and the float error would otherwise compound into shear. Flip is the hard case: a reflection has determinant `-1`, which inverts triangle winding at bake time, so the transform is composed as `R * basis * H` where `H` reflects along the local axis most aligned with the mirror normal — the two `-1`s cancel and the basis stays right-handed, while `H * H = I` keeps every world vertex exactly where the mirror puts it. Whether local face data has to change is decided empirically per brush: if reflecting the generated vertex set through `H` reproduces the same set, the primitive survives; otherwise the faces are promoted to `CUSTOM`, reflected and reversed. Surviving as a primitive does not mean the faces are left alone, though — face data is held by index and the mirror sends each face to where a *different* face used to be, so each face takes the data of the face its own reflection lands on, mirrored back into place. Without that, a material on the positive X face stayed on positive X after an X flip. Misjudging that test can only fall to the exact path, never to wrong geometry. Flip refuses brushes with displacement faces, because the displacement grid is indexed against its face's corner order. Pivots use the selection's origin centroid, not its bounding-box centre, because the centroid is exactly invariant under rotation and reflection about itself. `normalize_handedness()` folds the same way for a mirror Flip did not make: Godot's own scale gizmo will drag a handle past zero and the Inspector will take a typed negative, and a brush left like that bakes inside out — as a cutter it adds its volume instead of removing it. It runs at the two doors a brush transform arrives through, `create_brush_from_info()` and `HFBrushChangeTracker.reconcile()`, and the choice of local axis is cosmetic: all three give identical world geometry and identical per-face appearance, so it picks the axis the Inspector's own scale signs name when exactly one is negative, and X otherwise.
...
- **Geometry-aware snapping.** `_snap_point()` delegates to `HFSnapSystem`. Five modes (Grid=1, Vertex=2, Center=4, Edge=8, Perpendicular=16) form a bitmask. Custom snap lines (set via `set_custom_snap_line()`) are checked alongside grid/geometry candidates. Candidates come from a brush's own faces when it has them, so a wedge, prism, polygon, path or merged brush snaps to real corners rather than the corners of a box it does not fill. A brush past `MAX_SNAP_FACES` (64) is engine tessellation rather than geometry anyone could aim at, and keeps its bounding box. Either way the points are taken in brush space and carried through `global_transform`, so rotation and scale apply. Vertex collects those corners, Center collects brush centers, Edge collects edge midpoints, and Perpendicular projects the current point onto the closest point of each edge. Face geometry is cached per brush id, keyed on the node instance, face count and size, and dropped on `brush_changed` or `brush_removed`. Brush space does not move with the brush, so an entry survives every drag of everything around it. A brush whose own extent cannot reach within `snap_threshold` of the query point is skipped before its snap points are built at all, measured from the brush origin so a long wall is still reached by its far end. The closest eligible geometry candidate within `snap_threshold` beats grid snap. Pass `exclude_ids` to skip the brush being dragged.
...
- **GUT unit + integration tests** -- 4,473 tests across 243 test scripts (4,466 passing plus seven intentional no-assert safety tests; 20,813 assertions; verified in CI on September 19, 2026; runs Godot headless)
...
| `test_brush_info_roundtrip.gd` | 23 | Brush info capture/restore with visgroups, group_id, brush_entity_class, material, move floor/ceiling argument safety |
...
| `test_plugin_drop_handler.gd` | 4 | Supported drag payload classification, thin plugin delegates, placement/selection/prefab/material undo contracts, and a material dropped on a brush that has no id yet |
...
| `test_scoped_undo_step.gd` | 69 | The scoped undo step: what a scope records and refuses, in-place apply versus rebuild, order and connections across a rebuild, an entity record written back onto the live node it came from, every scoped command round-tripping against the whole level state, a collated run undoing to the start and redoing the whole way, and every scoped command changing nothing outside its scope |
