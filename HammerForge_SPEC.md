...
- **Placement space**: a `Node3D` that is not yet inside the tree has no parent to measure against, so assigning `global_position` or `global_transform` writes the local transform instead, and Godot logs an out-of-tree error on the read side. Anything created from world coordinates is therefore parented first and positioned second: `place_brush()` after the draft or pending container adopts the brush, `create_brush_from_info()`, `create_default_spawn()`, and `create_entity_from_map()` after `add_entity()`. `restore_entity_from_info()` assigns after `add_child()` and before it emits `entity_added`, so a listener never sees the entity at the wrong place. This matters whenever `LevelRoot` is away from the world origin, which is the case the ordering bugs were only ever invisible in. `tests/test_brush_system.gd` and `tests/test_entity_props.gd` hold the property with the root translated.
...
Full suite (verified in CI on September 9, 2026): **3,002 tests** across **157 scripts** (**2,995 passing** plus seven intentional no-assert safety tests; **15,703 assertions**).
