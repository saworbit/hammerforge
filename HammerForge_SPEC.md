...
- **Preview overlay space**: every preview container is a child of `LevelRoot`, so a mesh instance's *local* transform is in the root's space, not the world's. What the previews are handed is mostly world-space — a brush's own `global_transform`, a plane built from `world_bounds_of()`, an intersection from `world_aabb()` — and those are assigned through `global_transform`. The exception is `HFSubtractPreview`'s CSG results: `get_meshes()` reports transforms relative to a combiner parented to `LevelRoot`, so those are already in the root's space and stay local. `tests/test_preview_placement.gd` holds the property directly, with the root moved and turned.
