...
- **Relocation and hand edits**: each record keeps a per-piece signature of where the piece was put, how it was turned, and a hash of what it is. A structure whose surviving pieces all received the same rigid transform — a turn, a slide, or both — is relocated, and `relocation_transform()` is composed onto the placement so it rebuilds where it now stands and at the angle it now has; pieces that disagree were edited individually. A move that is not rigid answers no: a squash is not a relocation, and a mirror is refused outright because rebuilding through a negative-determinant basis would invert the winding of every face and not look wrong until the bake. A piece is counted as edited when its shape hash differs — asked in the basis it was *recorded* in, so a structure turned as a whole is not mistaken for a structure reshaped by hand — or when its transform is not where the relocation would have put it. Signatures written before piece bases existed have none; the reader answers a missing basis with the placement's own, which is what every piece the generator built was given, so an older record recovers a turn without a migration.
