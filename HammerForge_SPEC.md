...
- **Structure section** (Ctrl+Shift+A, dispatched from the dock, the command palette and the viewport): builds whatever type the section is showing, placed on the selection or the world origin. The shortcut keeps the action id `create_arch` so existing custom keymaps still resolve. `HFGeneratorSystem.can_build()` is asked before any undo action is opened, because each field can be in range while the combination is not — a wall as thick as the arch is wide, an arc of zero, a wide arc across too few segments. A refused build says why and records nothing.
