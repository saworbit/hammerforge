...
- **Selection Tools** (visible when brushes are selected, grouped by domain):
  ...
  - **Entity Binding**: Tie/Untie brush entity class (populated from entity definitions), with an optional name. The name is the address an I/O connection targets, so a door has to have one before anything can open it. Brushes tied in one go are one entity whether or not they are named, so a two-leaf door exports as one `func_door` rather than two that move independently; tying two doors separately keeps them separate.
...
**A door that moves**: a `func_door` or `door_basic` bakes into a holder of its own, carrying the mesh and the collision together, with a script that reads the class's `speed`, `wait` and `angle`. `Open` slides it its own width along the angle at `speed` metres a second, `wait` seconds later it closes itself (zero or less means it stays open), `Close` puts it back and `Toggle` does whichever is next. `Lock` and `Unlock` decide whether `Open` is listened to. It raises `OnOpen` and `OnClose` as it goes, so a door can drive a light or a sound. Brushes tied under one name are one door, and each leaf moves on its own, which is what a two-leaf door should do.
