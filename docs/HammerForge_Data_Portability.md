...
- Definitions include `classname`, `description`, `color`, `is_brush_entity`, `properties`, and the optional `class` and `scene`, which name the Godot node or `PackedScene` a playtest export builds for the entity. A property may carry `maps_to` naming the engine property its value is written to. The dock also reads presentation keys straight from the JSON: `label`, `preview`, and `category`.
