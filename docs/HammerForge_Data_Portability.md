...
- **Units are converted in both directions.** A `.map` file carries bare numbers and never says what a unit is. Every editor that writes one is on Quake units, where a player is 56 to 72 tall and the grid steps in 16s; HammerForge is on Godot's metric scale, where the playtest player is 1.6. The **Map units/m** row in the dock File section says how many `.map` units one HammerForge unit is worth, and it applies to import and to export. The default is **32**, which is the figure Func_Godot uses for the same exchange and which puts a 56-unit player at 1.75.
  - A 128 x 32 x 112 corridor imports as 4 wide, 3.5 high and 1 deep. A 12-metre room exports as 384 units, which is 24 of the other editor's grid squares.
  - Point entity origins take the same conversion as the plane points, because an origin is a position in the same space. Other numeric keys do not: a `func_door`'s `speed` is a distance per second in the source game's units, and only that game knows what it meant, so the keys travel as written.
  - The texture scale takes the conversion too. A `.map` reader divides the projected point by the scale field, so scaling the points and leaving the scale alone would tile the texture 32 times more often on a room that is otherwise the right size.
  - An export records the figure it used in `worldspawn`, as `_hf_units_per_metre`. An import reads it and uses it in preference to the dialog, so reopening your own export gives back the level you exported however the setting has moved since. A file from another editor states nothing and is read at whatever the row says. A figure of zero, a negative one or a non-number is refused and the row's value stands.
  - Changing the row between an import and an export rescales the level, on purpose. That is the way to move a map from one convention to another.
  - The conversion lives in `HFFileSystem`, not in `MapIO`. `MapIO.parse_map_text()` and `MapIO.export_map_from_level()` convert nothing unless a figure is passed, so a caller that wants the file's own numbers gets them.
- **The axes are converted in both directions too.** `.map` is Z-up across the whole Quake family; Godot is Y-up. Nothing converted, so a corridor drawn 112 units high arrived 3.5 metres deep and one metre high, and a floor exported from here opened in TrenchBroom as a wall. It went unnoticed for a long time because it is symmetric: out of HammerForge and back came home, and only the crossing was wrong.
  - The turn is a rotation about X: the file's `(x, y, z)` becomes `(x, z, -y)` here, and the reverse on the way out. Being a rotation, it leaves handedness alone, so the winding reversal below is unaffected by it.
  - That particular rotation is chosen so the level looks from above exactly as it did in the editor it came from. Func_Godot uses a cyclic `(y, z, x)`, which is equally correct and puts the map down at ninety degrees to the way the source editor drew it. That is noticeable the moment a piece is imported alongside geometry that is already there.
  - Point entity origins take the turn, for the same reason they take the scale. An `angle` does not: it is a compass bearing in the file's own horizontal plane and would need the turn applied to it as a direction rather than as a point, which is a per-key decision the importer does not make.
  - The Valve 220 texture axes take the turn as well. The reader computes the texture coordinate by projecting the point onto those axes, and rotating both by the same rotation leaves that projection unchanged, so the UVs come out of the conversion exactly as they went in.
  - An export records which way up it wrote, in `worldspawn`, as `_hf_axis_convention`: `quake` for the format's own axes and `godot` for this project's. A file that says `godot` is left alone on import. A file that says nothing is turned, because every editor other than this one writes the format's axes.
  - **A `.map` exported by a HammerForge build from before this change carries this project's axes and does not say so**, and there is nothing in such a file to tell it apart from one another editor wrote. Export it again from a current build, which records the convention, rather than relying on the import to guess.
...
`MapIO.parse_map_text()` automatically welds near-coincident parsed vertices before constructing brush geometry. The tolerance is controlled by `MapIO.import_weld_tolerance` (default **0.01**). Vertices within this distance are averaged to a shared position via BFS grouping over a spatial hash with 27-cell neighbor lookup, so pairs that straddle a snap-grid boundary are still caught.

The tolerance is in `.map` units and the weld runs before the unit conversion above, because the drift it exists to close is drift in the numbers as the other editor wrote them. At the default scale 0.01 of a map unit is about a third of a millimetre on the imported level.
