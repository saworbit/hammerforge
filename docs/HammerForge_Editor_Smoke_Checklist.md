...
- Give an entity an authored name that differs from its node name, aim an output at the authored name, then delete it. Confirm the connection goes. Do it again with a second entity sharing that authored name and confirm the connection stays, because it still resolves.
...
- Leave **Inference cleanup** off and paint an isolated cell plus a one-cell hole/gap pattern; confirm the authored cells are unchanged. Turn it on and repeat: confirm only isolated one-cell noise is removed, a cardinal one-cell hole/gap is filled, and a fast one-cell corridor gains one adjacent row. Confirm distant painted work and erase strokes are untouched. Then click a single cell on empty ground with cleanup still on and confirm the cell stays.
