...
- **`plugin.gd`** coordinates editor integration and routes to `LevelRoot`. Viewport arbitration, native RMB ownership, keyboard dispatch, vertex input, HUD state, commands, quick properties, and coach marks delegate to focused `plugin_*.gd` modules. It retains sticky `active_root` discovery with deep recursive tree search.
