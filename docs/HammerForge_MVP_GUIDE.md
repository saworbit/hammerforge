...
- **Arch** (Ctrl+Shift+A): `HFArchBuilder` builds a parametric arch, one brush per segment, placed on the selection or the world origin. Created through `HFGeneratorSystem`, which records the settings so the arch can be rebuilt later from the same dock section.
