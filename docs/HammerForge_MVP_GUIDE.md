...
- **`plugin.gd`** coordinates editor integration and routes to `LevelRoot`. Viewport arbitration, native RMB ownership, native object/Face Select pointer handling, EditorSelection synchronization and managed scope guards, undoable managed edit actions, entity/brush/prefab/material viewport drops, paint/draw/extrude pointer handling, numeric dimension entry, keyboard dispatch, vertex input, HUD state, command dispatch, overlay lifecycle/drawing, quick properties, and coach marks delegate to focused `plugin_*.gd` modules. It retains sticky `active_root` discovery with deep recursive tree search.
- **`level_root.gd`** is a 3,065-line coordinator that owns containers, exports, and signals. Public methods delegate to subsystem classes; continued decomposition is tracked in the roadmap. Default UX is Draw → material → entity → bake → Test Level; power-user overlays are opt-in.
...
- **Structure** (Ctrl+Shift+A): the Build tab's Structure section builds an **arch**, a **straight flight of stairs**, a **spiral staircase** or a **dome** from a handful of numbers, one brush per piece, placed on the selection or the world origin. The settings shown are the chosen builder's own — each describes itself through `HFGeneratorSchema` and the dock builds the controls from that.
- Every structure is created through `HFGeneratorSystem`, which records the settings so it can be rebuilt later from the same section — where you moved it to, and after telling you how many pieces a rebuild would overwrite.
- **Clip** (Shift+X): splits a brush along a plane through `HFConvexClip`. Any convex brush at any rotation; a piece that is still an axis-aligned box is emitted as a BOX so it keeps its handles. **Clip to Face Plane** (Shift+Alt+X) cuts along a selected face's plane. Preserves material, entity class, visgroups, group ID.
...
- **Rotate** (R / Shift+R) and **Flip** (Shift+M): `HFTransformSystem` turns and mirrors brushes and entities. Mirroring folds the reflection through a local axis so the basis keeps a positive determinant and geometry never bakes inside out. A negative scale set with Godot's own gizmo or typed into the Inspector is folded off the same way, so a brush cannot be left mirrored by either route; the brush stays exactly where it is and keeps the texturing on the same faces, and the scale field reads positive afterwards. **Reset Rotation** (Alt+R) clears a rotation, folding a quarter turn into `size` so nothing moves, and leaves any scale set with Godot's own gizmo alone. Hollow, clip and carve all work in a brush's own frame, so it is a tidying command rather than a prerequisite for any of them.
