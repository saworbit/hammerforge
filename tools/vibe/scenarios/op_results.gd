...
func _what_the_refusals_say() -> void:
	...
	var cases: Array = [
		...
		[
			"merge two brushes that do not touch",
			root.can_merge_brushes([str(a.brush_id), str(b.brush_id)])
		],
...
func _guards_that_say_yes(root: Node3D, a: Node, b: Node) -> void:
	...
	if not root.vertex_system.validate_convexity(survivor):
		...
		note(
			".map written from the merged brush",
			"%s bytes, %s plane points" % [text.length(), planes]
		)
		...
		flag(
			...
			(
				...
				% [
					without,
					without + with_hint,
					roundi(100.0 * without / float(without + with_hint))
				]
			)
		)
	...
	for path in HANDLER_SOURCES:
		...
		for i in lines.size():
			...
			if (
				...
				drops.append(
					"%s:%d %s" % [path.get_file(), i + 1, line.strip_edges().substr(0, 70)]
				)
