...
## anywhere in the level turns off the face-material path for the entire bake.
		...
		flag(
			...
			(
				(
					...
					+ "subtractive brush -- a window in one wall -- re-bakes the same six with "
					+ "%d. `_has_effective_structural_subtractors()` is a level-wide test, so "
					+ "any cutter anywhere turns `use_face_materials` off for every brush in the "
					+ "map, not just the ones the cut touches. On a real map, where cutting a "
					+ "vent is routine, the level loses its texturing the moment the first cut "
					+ "goes in."
				)
				% [distinct_before.size(), distinct_after.size()]
			)
		)
...
## The other half of a finding like this: whether anything tells the mapper.
func _what_the_mapper_is_told() -> void:
	...
	if mentions.is_empty():
		flag(
			"nothing tells the mapper the bake stopped using their face materials",
			(
				"hf_bake_system.gd:732 writes 'Face-material bake switched to CSG to preserve "
				+ "active cuts' to `root._log()`, which reaches the Console tab and nowhere "
				+ "else. The mirror-image case one branch below -- Use Face Materials switched "
				+ "off by hand while faces carry materials -- does emit a `user_message`. The "
				+ "case a mapper actually hits by accident is the silent one."
			)
		)
